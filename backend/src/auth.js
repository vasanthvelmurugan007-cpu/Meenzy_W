...
async function ensureTables() {
  ...
  try {
    ...
    // Ensure meenzy_preorders has the address_line column
    try {
      await client.query(`ALTER TABLE coexistence.meenzy_preorders ADD COLUMN IF NOT EXISTS address_line TEXT`);
    } catch (err) {
...
// POST /api/auth/agent-login (Fallback for compiled APK that sent 'password' instead of 'pin' to the wrong route)
router.post('/auth/agent-login', async (req, res) => {
  ...
  try {
    ...
    // Must use the same JWT_SECRET
    const token = jwt.sign({ id: agent.id, role: 'agent' }, process.env.JWT_SECRET || 'forgecrm-secret-key-123', { expiresIn: '30d' });
