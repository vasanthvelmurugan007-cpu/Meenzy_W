...
async function resolveAccount({ accountId, fromPhoneNumber }) {
  try {
    ...
    if (accountId) acc = await getAccountWithToken(accountId);
      ...
      acc = await getAccountByPhoneNumber(fromPhoneNumber);
    }
    ...
    if (!acc && process.env.META_PERMANENT_TOKEN && process.env.META_PHONE_NUMBER_ID) {
      acc = {
        id: 'env-fallback',
        displayName: 'Meenzy System',
        displayPhoneNumber: process.env.META_PHONE_NUMBER_ID,
        phoneNumberId: process.env.META_PHONE_NUMBER_ID,
        accessToken: process.env.META_PERMANENT_TOKEN,
        isActive: true
      };
    }
    if (!acc) return { error: `No account registered for ${fromPhoneNumber || `id=${accountId}`}` };
    if (!acc.isActive) return { error: `Account "${acc.displayName}" is inactive` };
    return { account: acc };
  } catch (err) {
  ...
  await pool.query(
    `INSERT INTO coexistence.chat_history
       (message_id, phone_number_id, wa_number, contact_number, to_number,
        ...
        media_url, media_mime_type, status, timestamp, template_meta, context_message_id, platform)
