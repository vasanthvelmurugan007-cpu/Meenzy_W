...
const meenzySessions = {};

async function handleMeenzyInteractiveButton(client, contactNumber, btnId, account, resolveAccount, insertPendingRow, enqueueSend) {
  // Initialize session if it doesn't exist
  if (!meenzySessions[contactNumber]) {
    console.log(`[meenzy-interactive] No active session found for customer ${contactNumber}`);
    return false;
  }

  const session = meenzySessions[contactNumber];

  let cutValue = null;
  if (btnId === 'cut_slices') cutValue = "Slices / Fry Cut";
  else if (btnId === 'cut_whole') cutValue = "Whole Cleaned";
  else if (btnId === 'cut_curry') cutValue = "Curry Cubes";

  let qtyValue = null;
  if (btnId === 'qty_05') qtyValue = 0.5;
  else if (btnId === 'qty_10') qtyValue = 1.0;
  ...
  if (cutValue) {
    session.selectedCut = cutValue;
    console.log(`[meenzy-interactive] Customer ${contactNumber} selected cut: ${cutValue}`);
  }

  if (qtyValue) {
    session.selectedQty = qtyValue;
    console.log(`[meenzy-interactive] Customer ${contactNumber} selected quantity: ${qtyValue}`);
  }

  if (slotValue) {
    session.selectedSlot = slotValue;
    console.log(`[meenzy-interactive] Customer ${contactNumber} selected slot: ${slotValue}`);
  }

  // Check if all options are selected
  if (session.selectedCut && session.selectedQty !== null && session.selectedQty !== undefined && session.selectedSlot) {
    const matchedFish = session.matchedFish;
    const selectedCut = session.selectedCut;
    const selectedQty = session.selectedQty;
    const selectedSlot = session.selectedSlot;
    const pricePerKg = session.priceInInr;

    const calculatedPrice = selectedQty * pricePerKg;
    ...
    console.log(`[meenzy-cart] Added to cart: customer=${contactNumber}, item=${matchedFish} (${selectedCut}), qty=${selectedQty}`);

    // Reset current selection so they can add more
    session.matchedFish = null;
    session.selectedCut = null;
    session.selectedQty = null;
    session.selectedSlot = null;
    session.priceInInr = null;

    const cartTotal = session.cart.reduce((sum, item) => sum + item.price, 0);

    // Send summary and Add More / View Cart options
    if (account) {
      const summaryText = `🛒 *Item Added to Cart!*\n\nAdded: *${matchedFish} (${selectedCut})*\nQuantity: *${selectedQty} kg*\nPrice: *₹${calculatedPrice}*\n\nTotal Cart Items: ${session.cart.length}\nTotal Cart Value: *₹${cartTotal}*`;
      ...
      const buttonPayload = {
        ...
        action: {
          buttons: [
            ...
            { type: "reply", reply: { id: "btn_view_cart", title: "View Cart & Confirm" } }
          ]
        }
      };
      ...
      await enqueueSend({
        ...
        localMessageId: localId,
        payload: { interactive: buttonPayload },
      });
    }
    return true;
  } else {
    // Send refreshed single-menu list payload showing currently selected options and what is remaining
    if (account) {
      const pricePerKg = session.priceInInr || 0;
      let priceDisplay = "";
      if (session.selectedQty !== null && session.selectedQty !== undefined) {
        const calculatedPrice = session.selectedQty * pricePerKg;
        const weightText = session.selectedQty === 0.5 ? '500g' : `${session.selectedQty * 1000}g`;
        const cutText = session.selectedCut || 'No Cut Selected';
        priceDisplay = `Selected: ${session.matchedFish} (${cutText}) | ${weightText}\n💰 Estimated Cost: ₹${calculatedPrice} (Based on ₹${pricePerKg}/Kg)`;
      } else {
        priceDisplay = `Price: ₹${pricePerKg}/Kg`;
      }

      const currentSelectionText = `Selected:\n• Cut: *${session.selectedCut || 'None'}*\n• Weight: *${session.selectedQty ? session.selectedQty + ' Kg' : 'None'}*\n• Delivery Slot: *${session.selectedSlot || 'None'}*`;
      
      let guidance = "";
      if (!session.selectedCut) {
        guidance = "Please select a *Cutting Option*! 🔪";
      } else if (session.selectedQty === null || session.selectedQty === undefined) {
        guidance = "Please select a *Weight Option*! ⚖️";
      } else if (!session.selectedSlot) {
        guidance = "Please select your preferred *Delivery Slot*! 🚚";
      }
      ...
      const listPayload = {
        ...
        action: {
          ...
          sections: [
            {
              title: "1. Cutting Options",
              rows: [
                { id: "cut_slices", title: "Slices / Fry Cut" },
                { id: "cut_whole", title: "Whole Cleaned" },
                { id: "cut_curry", title: "Curry Cubes" }
              ]
            },
            {
              title: "2. Weight Options",
              rows: [
                { id: "qty_05", title: "0.5 Kg" },
                { id: "qty_10", title: "1.0 Kg" },
                { id: "qty_20", title: "2.0 Kg" }
              ]
            },
            {
      ...
      const localId = await insertPendingRow({
        ...
        toNumber: contactNumber,
        messageType: 'interactive',
        messageBody: `Great choice! 🐟 Let's customize your fresh order of ${session.matchedFish}. Please select options below:`,
      });
      await enqueueSend({
        kind: 'interactive',
        accountId: account.id,
      ...
      console.log(`[meenzy-interactive] Sent refreshed customize menu list payload to: ${contactNumber}`);
    }
    return true;
  }
}

async function run7PmCatalogBroadcast() {
  const { getSingleAccount } = require('../routes/whatsappAccounts');
  const { fetchCatalogProducts } = require('../routes/webhook');

  try {
    console.log('[meenzy-broadcast] Starting daily 7:00 PM broadcast...');
    const account = await getSingleAccount();
    if (!account || !account.accessToken || !account.phoneNumberId) {
      console.error('[meenzy-broadcast] Error: Active WhatsApp account details not found.');
      return;
    }

    const contactsRes = await pool.query(
      `SELECT DISTINCT contact_number FROM coexistence.contacts WHERE deleted_at IS NULL`
    );
    const contacts = contactsRes.rows;
    if (contacts.length === 0) {
      console.log('[meenzy-broadcast] No active contacts found for broadcast.');
      return;
    }

    // Fetch products from Wix TSV feed
    let wixProducts = [];
    try {
      wixProducts = await fetchCatalogProducts();
    } catch (e) {
      console.error('[meenzy-broadcast] Failed to fetch Wix catalog products:', e.message);
    }

    const catalogId = process.env.MEENZY_CATALOG_ID || "2150289245547170";

    const steaks = wixProducts.filter(p => p.name.toLowerCase().match(/seer|vanjaram|salmon|tuna|steak|fillet/i)).slice(0, 5);
    const locals = wixProducts.filter(p => p.name.toLowerCase().match(/pomfret|rohu|catla|mackerel|sardine/i)).slice(0, 5);
    const shellfish = wixProducts.filter(p => p.name.toLowerCase().match(/prawn|crab|squid|lobster|shrimp/i)).slice(0, 5);

    const sections = [];
    if (steaks.length > 0) {
      sections.push({
        title: 'Premium Steaks',
        product_items: steaks.map(p => ({ product_retailer_id: p.retailer_id || p.id }))
      });
    }
    if (locals.length > 0) {
      sections.push({
        title: 'Local Favorites',
        product_items: locals.map(p => ({ product_retailer_id: p.retailer_id || p.id }))
      });
    }
    if (shellfish.length > 0) {
      sections.push({
        title: 'Shellfish',
        product_items: shellfish.map(p => ({ product_retailer_id: p.retailer_id || p.id }))
      });
    }
    ...
    for (const contact of contacts) {
      const toNumber = contact.contact_number;
      try {
        let payload;
        // Try Multi-Product Message first if sections exist
        if (sections.length > 0) {
          payload = {
            ...
            interactive: {
              ...
              header: {
                type: 'text',
                text: 'Meenzy Fresh Seafood'
              },
              body: {
                text: '🌊 Order the freshest live seafood for your dinner feast tonight! Browse our selection:'
              },
              footer: {
                text: 'Meenzy Seafood'
              },
          ...
          // Fallback to catalog root button message
          payload = {
            messaging_product: 'whatsapp',
            recipient_type: 'individual',
            to: String(toNumber).replace(/\D/g, ''),
            ...
            interactive: {
              type: 'catalog_message',
              body: {
                text: '🌊 Order the freshest live seafood for your dinner feast tonight! Click below to view catalog:'
              },
              action: {
                name: 'catalog_message',
                parameters: {
                  thumbnail_product_retailer_id: wixProducts[0]?.retailer_id || wixProducts[0]?.id || ''
                }
              }
            }
          };
        ...
        if (!res.ok) {
          console.warn(`[meenzy-broadcast] Interactive send failed for ${toNumber} (falling back to template):`, parsed?.error?.message || resText);
          
          // Fallback to Catalog Root button template message
          ...
          const tResText = await tRes.text();
          let tParsed = null;
          try { tParsed = JSON.parse(tResText); } catch {}

          if (!tRes.ok) {
            console.error(`[meenzy-broadcast] Template fallback also failed for ${toNumber}:`, tParsed?.error?.message || tResText);
          } else {
            console.log(`[meenzy-broadcast] Template fallback sent successfully to ${toNumber}`);
            // Log template in chat_history
            const wamid = tParsed?.messages?.[0]?.id || `local-bc-${Date.now()}`;
            await pool.query(
              ...
              [
                ...
                String(toNumber).replace(/\D/g, ''),
                'Sent daily catalog template broadcast fallback',
                'sent'
              ]
            );
          }
        } else {
          console.log(`[meenzy-broadcast] Interactive broadcast sent successfully to ${toNumber}`);
          // Log interactive in chat_history
          const wamid = parsed?.messages?.[0]?.id || `local-bc-${Date.now()}`;
          await pool.query(
            `INSERT INTO coexistence.chat_history 
             (message_id, phone_number_id, wa_number, contact_number, to_number, direction, message_type, message_body, status, timestamp)
             VALUES ($1, $2, $3, $4, $5, 'outgoing', 'interactive', $6, $7, NOW())
             ON CONFLICT (message_id) DO NOTHING`,
            [
              wamid,
              account.phoneNumberId,
              ...
              String(toNumber).replace(/\D/g, ''),
              String(toNumber).replace(/\D/g, ''),
              'Sent daily catalog interactive broadcast',
              'sent'
            ]
          );
        }
      } catch (e) {
        console.error(`[meenzy-broadcast] Exception sending broadcast to ${toNumber}:`, e.message);
      }
    }
  } catch (err) {
    console.error('[meenzy-broadcast] Fatal broadcast error:', err.message);
  }
}

// Set up daily cron job at 19:00 local time
const cron = require('node-cron');
cron.schedule('0 19 * * *', async () => {
  try {
    await run7PmCatalogBroadcast();
  } catch (err) {
    console.error('[meenzy-scheduler] Error running daily catalog broadcast cron:', err.message);
  }
}, {
...
module.exports = {
  ...
  evaluateConditions,
  meenzySessions,
  handleMeenzyInteractiveButton,
  run7PmCatalogBroadcast,
};
