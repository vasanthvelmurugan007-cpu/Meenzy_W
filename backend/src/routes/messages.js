...
const { sendText, sendMedia, sendTemplate, sendInteractive, sendLocation, sendContacts } = require('../integrations/metaSend');
...
router.get('/numbers', async (req, res) => {
  try {
    ...
    const enriched = rows.map((row) => {
      ...
      let name = teamMember?.name || nameMap[row.wa_number] || 'Meenzy';
      if (!name || name === row.wa_number) {
        name = 'Meenzy';
      }
      return {
        ...row,
