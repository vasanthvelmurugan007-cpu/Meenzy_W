...
async function extractOrderLLM(messageText) {
  try {
    const apiKey = process.env.OPENROUTER_API_KEY || process.env.GEMINI_API_KEY;
    if (!apiKey) return [];
...
Never return an empty array if any fish name is mentioned. Extract it even if it's misspelled.
If no order is found at all, return an empty array [].
      ...
      for (const r of allRecords) {
        ...
        if (r.direction === 'incoming' && r.message_type === 'location' && r.latitude && r.longitude) {
          const updateAgentRes = await client.query(
            `UPDATE coexistence.meenzy_delivery_agents
        ...
        if (r.direction === 'incoming' && r.message_type === 'image' && !r.__handled) {
          const agentRes = await client.query(`SELECT id, name FROM coexistence.meenzy_delivery_agents WHERE phone = $1`, [r.contact_number]);
             ...
             if (!r.__handled) {
               ...
               if (intent === 'ORDER_COMPLAINT') {
                 ...
                 r.__handled = true;
                  ...
                  const items = await extractOrderLLM(trimmedBody);
  ...
  try {
    ...
    if (!error && account) {
      ...
      const localId = await insertPendingRow({
        ...
        messageType: 'text',
        messageBody: 'Sent website order confirmation',
      });
      await enqueueSend({
        kind: 'text',
        accountId: account.id,
