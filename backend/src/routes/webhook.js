      ...
      for (const r of allRecords) {
        ...
        if (r.direction === 'incoming' && r.message_type === 'interactive' && r.selected_button_id) {
          ...
          if (!error && account) {
            ...
            if (btnId.startsWith('swap_alt_') || btnId === 'swap_cancel' || btnId === 'swap_postpone') {
              ...
              if (!session || session.state !== 'AWAITING_FAILURE_SWAP') {
                ...
                console.log(`[meenzy-swap] No active AWAITING_FAILURE_SWAP session for ${r.contact_number}. Skipping.`);
                r.__handled = true;
                continue;
              }
              ...
              // ── Swap Fish branch ──────────────────────────────────────────────
              // Parse which alternative was chosen (swap_alt_1 → index 0)
