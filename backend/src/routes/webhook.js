      ...
      for (const r of allRecords) {
        ...
        if (r.direction === 'incoming' && r.message_type === 'interactive' && r.selected_button_id) {
          ...
          if (!error && account) {
            ...
            // Handles list-reply IDs: swap_alt_1/2/3, swap_cancel, swap_postpone.
            ...
            if (btnId.startsWith('swap_alt_') || btnId === 'swap_cancel' || btnId === 'swap_postpone' || btnId === 'swap_confirm_yes' || btnId === 'swap_confirm_no') {
              const { meenzySessions } = require('../engine/automationEngine');
              const session = meenzySessions[r.contact_number];

              if (!session || (session.state !== 'AWAITING_FAILURE_SWAP' && session.state !== 'AWAITING_SWAP_CONFIRMATION')) {
              ...
              const quantity          = parseFloat(session.originalQuantity) || 1;
              ...
              // ── Swap Confirmation: NO ──────────────────────────────────────────
              if (btnId === 'swap_confirm_no') {
                session.state = 'AWAITING_FAILURE_SWAP';
                delete session.selectedAltIndex;
                
                // Re-send the original options
                const optionsRows = replacements.map((alt, idx) => {
                ...
                const listPayload = {
                  type: "list",
                  header: { type: "text", text: `⚠️ Swap Cancelled` },
                  body: { text: `Please select another option for your order of *${quantity} Kg* of *${oldFishName}*:` },
                  action: { button: "Resolve Order", sections: [{ title: "Choose Resolution Path", rows: optionsRows }] }
                };

                const localId = await insertPendingRow({ account, toNumber: r.contact_number, messageType: 'interactive', messageBody: `Please choose a different resolution.` });
              ...
              // ── Swap Fish branch (Selection) ───────────────────────────────────
              if (btnId.startsWith('swap_alt_')) {
                const altIndex = parseInt(btnId.replace('swap_alt_', ''), 10) - 1;
                const selectedAlt = replacements[altIndex];

                if (!selectedAlt) {
                  console.warn(`[meenzy-swap] Unknown swap alternative "${btnId}" for ${r.contact_number}`);
                  r.__handled = true;
                  continue;
                }
                ...
                if (priceDiff !== 0) {
                  // Require confirmation for price change
                  session.state = 'AWAITING_SWAP_CONFIRMATION';
                  session.selectedAltIndex = altIndex;

                  // Check payment status from ecosystem_orders if exists
                  ...
                  try {
                    ...
                    if (ecoRes.rows.length > 0) {
                      paymentStatus = ecoRes.rows[0].payment_status || 'PENDING';
                    }
                  } catch (err) {
                    console.warn('[meenzy-swap] Could not fetch payment status:', err.message);
                  }

                  let confirmText = `You have selected *${newFishName}* to replace *${oldFishName}*.\n\n`;
                  if (priceDiff > 0) {
                    confirmText += `This item costs more (₹${newFishPrice}/Kg vs ₹${oldFishPrice}/Kg). `;
                    if (paymentStatus === 'PAID' || paymentStatus === 'COLLECTED') {
                      confirmText += `An additional balance amount of *₹${priceDiff}* will need to be paid on delivery.`;
                    } else {
                      confirmText += `Your new cash-on-delivery total will be *₹${newTotal}* (an increase of ₹${priceDiff}).`;
                    }
                  } else {
                    const saved = Math.abs(priceDiff);
                    confirmText += `This item is cheaper (₹${newFishPrice}/Kg vs ₹${oldFishPrice}/Kg). `;
                    if (paymentStatus === 'PAID' || paymentStatus === 'COLLECTED') {
                      confirmText += `A refund/credit of *₹${saved}* will be processed for the difference.`;
                    } else {
                      confirmText += `Your new cash-on-delivery total will be reduced to *₹${newTotal}* (savings of ₹${saved}).`;
                    }
                  }
                  ...
                  const interactivePayload = {
                    ...
                    action: {
                      buttons: [
                        { type: "reply", reply: { id: `swap_confirm_yes`, title: "Yes, Confirm Swap ✅" } },
                        { type: "reply", reply: { id: `swap_confirm_no`, title: "No, Go Back ❌" } }
                      ]
                    }
                  };

                  const localId = await insertPendingRow({ account, toNumber: r.contact_number, messageType: 'interactive', messageBody: `Swap confirmation required for ${newFishName}` });
                  await enqueueSend({ kind: 'interactive', accountId: account.id, to: String(r.contact_number).replace(/\D/g, ''), localMessageId: localId, payload: { interactive: interactivePayload } });
                  r.__handled = true;
                  continue;
                } else {
                  // Price is the same, no confirmation needed
                  session.selectedAltIndex = altIndex;
                  btnId = 'swap_confirm_yes';
                }
              }

              // ── Swap Fish branch (Execution) ───────────────────────────────────
              if (btnId === 'swap_confirm_yes') {
                const altIndex = session.selectedAltIndex;
                const selectedAlt = replacements[altIndex];

                if (!selectedAlt) {
                  console.warn(`[meenzy-swap] Unknown swap alternative index "${altIndex}" for ${r.contact_number}`);
                  r.__handled = true;
                  continue;
                }
                ...
                // ── Build the customer-facing notification message ─────────────────
                let swapMsg;
                ...
                if (newFishPrice > oldFishPrice) {
                  ...
                  swapMsg =
                    ...
                    `Because *${newFishName}* costs more (₹${newFishPrice}/Kg vs ₹${oldFishPrice}/Kg), ` +
                    `an additional amount of *₹${extraAmount}* will be added to your order.\n\n` +
                    `📦 *Order Summary:*\n` +
                  ...
                  swapMsg =
                    `🔄 *Swap Request Confirmed!*\n\n` +
                    `You've selected *${newFishName}* to replace *${oldFishName}*.\n\n` +
                    `Great news — *${newFishName}* is cheaper (₹${newFishPrice}/Kg vs ₹${oldFishPrice}/Kg)! ` +
                    `A credit of *₹${savedAmount}* will be applied to your account.\n\n` +
                    `📦 *Order Summary:*\n` +
                    `• Item: ${newFishName} (${quantity} Kg)\n` +
                    `• Rate: ₹${newFishPrice}/Kg\n` +
                    `• Previous total: ₹${oldTotal}\n` +
                  ...
                  swapMsg =
                    `🔄 *Swap Request Confirmed!*\n\n` +
                    `You've selected *${newFishName}* to replace *${oldFishName}*.\n\n` +
                    `✅ *Same price — no extra charge!* (₹${newFishPrice}/Kg)\n\n` +
                    `📦 *Order Summary:*\n` +
                    `• Item: ${newFishName} (${quantity} Kg)\n` +
                    `• Rate: ₹${newFishPrice}/Kg\n` +
                    `• Total: *₹${newTotal}*\n\n` +
                ...
                await enqueueSend({ kind: 'text', accountId: account.id, to: String(r.contact_number).replace(/\D/g, ''), localMessageId: localId, payload: { body: swapMsg, previewUrl: false } });
                r.__handled = true;
                continue;
              }
