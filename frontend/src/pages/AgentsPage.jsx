...
export default function AgentsPage() {
  ...
  const [newAgent, setNewAgent] = useState({ name: '', phone: '', vehicle_info: '', pin: '' });
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      setIsAdding(false);
      setNewAgent({ name: '', phone: '', vehicle_info: '', pin: '' });
      fetchAgents();
    } catch (err) {
      ...
      {isAdding && (
        <form onSubmit={handleCreate} style={{ background: '#fff', padding: 24, borderRadius: 12, boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: 24, border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#4b5563', marginBottom: 4 }}>Login PIN</label>
              <input style={inputStyle} type="password" value={newAgent.pin} onChange={e => setNewAgent({...newAgent, pin: e.target.value})} placeholder="e.g. 1234" required minLength="4" />
            </div>
          </div>
